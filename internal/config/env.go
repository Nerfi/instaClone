...
func ParseEnvs() (*AppEnvs, error) {
	...
	envOnce.Do(func() {
		...
		if parseErr != nil {
			fmt.Println("Error parsing HTTP_COOKIE_HTTPONLY", "err", parseErr)
			err = parseErr
			return
		}

		Envs.HTTP_COOKIE_SECURE = httpCookieHttpOnly

		httpCookieSecure, parseErr := strconv.ParseBool(os.Getenv("HTTP_COOKIE_SECURE"))
		if parseErr != nil {
			fmt.Println("Error parsing HTTP_COOKIE_SECURE", "err", parseErr)
			err = parseErr
			return
		}
		...
		if parseErr != nil {
			fmt.Println("Error parsing HTTP_REFRESH_TOKEN_EXPIRE", "err", parseErr)
			err = parseErr
			return
		}
		...
		if parseErr != nil {
			fmt.Println("Error parsing HTTP_ACCESS_TOKEN_EXPIRE", "err", parseErr)
			err = parseErr
			return
		}
	...
	if Envs.ENV == "development" {
		fmt.Println("env variables", "env", Envs)
	}
	fmt.Println("envs parsed successfully")
